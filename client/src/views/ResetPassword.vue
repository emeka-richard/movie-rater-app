<template>
  <form @submit.prevent="reset">
    ...
    <small
      ><RouterLink class="link" to="/login">Back to login</RouterLink></small
    >
  </form>
</template>
...
const reset = async () => {
  ...
  const userEmailforNewPassword = {
    email: user.value.email,
  };
  store
    .dispatch("reset", userEmailforNewPassword)
    .then(() => {
      router.push({ name: "emailnotice" });
    })
    .catch((errMSG) => {
